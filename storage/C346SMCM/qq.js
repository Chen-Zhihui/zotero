(function(a, c) {
	...
	var l = function(s, v, r) {
		var q, t = 0, u = s.length, p = u === c
				|| Object.prototype.toString(s) === "[object Function]";
		if (r) {
			if (p) {
				for (q in s) {
		...
		return i = {
			...
			isPlainObject : function(u) {
				...
				try {
					...
					if (u.constructor
							&& !r.call(u, "constructor")
							&& !r
									.call(u.constructor.prototype,
											"isPrototypeOf")) {
						return false
					}
				} catch (t) {
	...
	var g = function() {
		var p = {};
		return function(t) {
			var s = +new Date(), r = "log_" + s, q = p[r] = new Image();
	...
	var f = function() {
		...
		var r = i;
		return function(v) {
			var u = v.url || p, x = v.level || t, w = (r.isString(v) ? v
					: v.info)
					|| "Empty info!";
			if (e.debug) {
	...
	var h = function(v, r, p) {
		...
		v = d(v);
		var u = v.split("."), q = u.pop(), t = n, w, s;
		while (u.length > 0) {
			w = u.shift();
			t[w] = t[w] || {};
			if (i.isNumeric(t[w]) || typeof t[w] === "boolean") {
				throw new TypeError(
						"define: cannot attach stuffs to number or boolean!")
			}
			t = t[w]
		}
		r = b(r);
	...
	h("util.speedReport", "util.report", function(r) {
		...
		var q = function(t, s, v, u) {
			this.url = "http://isdspeed.qq.com/cgi-bin/r.cgi?flag1=" + t
					+ "&flag2=" + s + "&flag3=" + v + "&";
			this.p0 = +new Date();
			this.points = [ this.p0 ];
			this.rate = u || 1
	...
	h("util.getScript", function() {
		return function(q, s) {
			var p = document.createElement("script"), r = document
	...
	h("util.combo", "globalSettings,util.getScript", function(q) {
		var s = q("globalSettings"), p = q("getScript");
		var r = 20;
		return function(v, y, A) {
			for ( var x = 0, t = Math.ceil(y.length / r), z, u; x < t; x++) {
				z = r * x;
				u = t === x + 1 ? y.length : z + r;
				var w = v + s.comboTag + y.slice(z, u).join(s.comboDelimiter)
						+ "?v=" + s.version;
				p(w, A)
			}
		}
	...
	h("util.getRequireList", "globalSettings", function(p) {
		...
		var r = s.srcMap, q = s.moduleCache;
		return function(t) {
			if (!t || t.split(",").length === 0) {
				return ""
			}
			var y = [], x = t.split(","), w = 0, v = {}, z, u;
			while (z = x[w++]) {
	...
	h(
			"api.require",
			"globalSettings,lang.each,lang.trim,lang.typeEnhance,util.pubSub,util.combo,util.getRequireList",
			function(s) {
				var w = s("globalSettings"), t = s("typeEnhance"), u = s("each"), p = s("trim"), q = s("pubSub"), v = s("combo"), r = s("getRequireList");
				return a.require = function(y, C) {
					...
					if (!t.isString(y)) {
						throw new TypeError(
								"api.require: require should be string!")
					}
					if (typeof C !== "undefined" && !t.isFunction(C)) {
						throw new TypeError(
								"api.require: callback should be function!")
					}
					var x = r(y);
					if (x.length === 0) {
						C && C(b(y))
	...
	h("api.define", "lang.typeEnhance,api.require,util.pubSub", function(q) {
		var s = q("typeEnhance"), r = q("api.require"), p = q("util.pubSub");
	...
	h(
			"api.load",
			"api.require,globalSettings,util.speedReport",
			function(s) {
				var B = s("require"), t = s("globalSettings"), r = s("speedReport");
				var z = "0", v = "1", y = "2", A = "4", x = "20", u = "0", w = "1", q = "2";
				var p = t.global;
				return p.load = function(C) {
					...
					C.insert = function() {
						var E = document.getElementsByTagName("script"), F = E[E.length - 1];
						return function(G) {
					...
					var D = r("7818", "21", "1");
					B(
							"util.speedReport,util.domain,wpa.wpaMgr,wpa.visitor,wpa.kfuin",
							function(E) {
								D.addPoint(2).send();
								var J = E("wpaMgr"), I = E("visitor"), H = E("domain"), K = E("speedReport"), F = E("kfuin");
								var G = C.nameAccount;
								if (!G) {
									return
								}
	...
	h(
			"wpa.APIs",
			"globalSettings,lang.each,api.require,util.pubSub,util.speedReport",
			function(s) {
				var t = s("globalSettings"), w = s("each"), x = s("require"), v = s("pubSub"), r = s("speedReport");
				var u = "wpa.APIs.";
				var q = t.global;
				var p = [ "add", "addCustom", "on", "visitor", "gdtChat",
						"link" ];
				w(p, function() {
					q[this] = function(y) {
						return function() {
							var z = arguments, A = u + y;
							var B = r("7818", "21", "1");
